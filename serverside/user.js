...
router.post('/', (request, response) => {
    ...
    const connection = db.connect1()
    const statement = `
    insert into mrs (username, firstname, lastname, phoneno, email, password) values('${username}', '${firstname}', '${lastname}', '${phoneno}', '${email}', '${password}')`
    connection.query(statement, (error, data) => {
        connection.end()
        response.send(utils.createResult(error, data))
    })
...
router.put('/edit_user/:id',(request,response)=>{
    ...
    const statement = `update mrs set username='${username}',firstname='${firstname}',lastname='${lastname}',phoneno='${phoneno}',email='${email}',password='${password}' where id =${id}`
